...
# Enqueue a priority Bitcoin Core Guix build with an optional build-name description
[group('live')]
guix-submit revision description='' host=default_host:
    @[[ {{quote(revision)}} =~ ^[0-9a-fA-F]{40}$ ]] || { echo "error: revision must be a full 40-character hexadecimal commit ID" >&2; exit 2; }
    ssh {{quote(host)}} {{quote("sudo -u ci-runner ci-runner --queue-dir /var/lib/ci-runner/queue enqueue bitcoin-guix --kind manual --revision " + quote(revision) + " --description " + quote(description))}}
